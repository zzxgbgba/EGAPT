...
public class RotateSchedule {
    ...
    private int rotateIndex = 0;//轮询下标

    public RotateSchedule(VMID[] vmIDArray, ArrayList<PhyMachine> phyMachineList, TimeLine timeLine, int lessonCount) {
    ...
    public PhyPosInfo[] getScheduleByGA()//对应每一个虚拟机开在某一个物理机上
    {
        ...
        for (TimeBlock timeBlock : timeLine.getTime()) {
            ...
            while(flag == 0){
                ...
                locate = findNextPhy(timeBlock.getLessonList().get(randomIndex).getVmList().get(hasScheduled[lessonID]));
        ...
        System.out.println("Rotate调度算法结果:");
    ...
    public int findNextPhy(VirtualMachine vm ) {//搜索最小负载的物理机
        int i = 0;
        int flag = 0;
        int index = 0;
        for (i =0; i < phyMachineList.size(); i++) {

                if((phyMachineList.get(rotateIndex).getCpuUsed()+vm.getCpuRequest())/phyMachineList.get(rotateIndex).getCpuFrequency()<=1&&(phyMachineList.get(rotateIndex).getMemUsed()+vm.getMemRequest())/phyMachineList.get(rotateIndex).getMem()<=1){
                    index = rotateIndex;
                    flag = 1;
                    if(rotateIndex+1<phyMachineList.size())
                       rotateIndex++;
                    else
                        rotateIndex = 0;
