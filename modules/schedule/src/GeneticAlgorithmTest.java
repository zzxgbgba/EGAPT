...
public class GeneticAlgorithmTest {
    ...
    private PhyPosInfo[] schedule = null;//精英个体
    private PhyPosInfo[] history = null;//发生意外之前已经调度完的虚拟机

    public GeneticAlgorithmTest(int popSize, int gmax, double crossoverProb, double mutationRate, VMID[] vmIDArray, ArrayList<PhyMachine> phyMachineList, TimeLine timeLine, PhyPosInfo[] schedule, PhyPosInfo[] history){
       setPopSize(popSize);
       setGmax(gmax);
       ...
       for(PhyMachine phyMachine:this.phyMachineList){
    ...
    public ArrayList<PhyPosInfo[]> initPopsRandomly(int vmNum,int phyNum,int popsize)//参数代表虚拟机的个数、物理机的个数和要提供的种群个体数量
    {
        ...
        for(int i=0;i<popsize;i++)
        {
            ...
            for(int j=0;j<vmNum;j++)
            {
                ...
                if(vmIDArray[j].getIsLocked()==0)
                    schedule[j].setPhyID(new Random().nextInt(phyNum));
                else
                    schedule[j].setPhyID(this.history[j].getPhyID());
            }
        ...
        schedules.remove(popsize-1);
        schedules.remove(popsize-2);
        schedules.remove(popsize-3);
        schedules.remove(popsize-4);
        //schedules.remove(popsize-5);
        //schedules.remove(popsize-6);
        schedules.add(0,this.schedule.clone());

        schedules.add(popsize/4,this.schedule.clone());
        schedules.add(popsize/2,this.schedule.clone());
        schedules.add(popsize/4*3,this.schedule.clone());
        //schedules.add(this.schedule.clone());
        //schedules.add(this.schedule.clone());
    ...
    public void operateMutation(PhyPosInfo[] child)
    {
        ...
        if(vmIDArray[mutationIndex].getIsLocked()==0){
    ...
    private PhyPosInfo[] findBestSchedule(ArrayList<PhyPosInfo[]> pop)
    {
        ...
        System.out.println("加入精英个体的随机遗传算法结果:");
